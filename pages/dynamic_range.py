...
def main():
    ...
    circles = [
        Circle(center, detection_range, color="grey", fill=False, linestyle="--")
    ...
    circle_proxy = Line2D(
        [0], [0], linestyle="--", alpha=0.7, color="grey", label="Detection range"
    )
...
def get_input():
    ...
    n_sensors = st.sidebar.slider(
        "Number of sensors", 1, 5, n_sensors, step=1, help=help
    )
    if n_sensors > 1:
